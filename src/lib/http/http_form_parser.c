    ...
    while (token != NULL && form_data->count < MAX_FORM_FIELDS) {
        ...
        if (eq) {
            *eq = '\0';
            ...
            url_decode(form_data->fields[form_data->count].value, value);
            strncpy(form_data->fields[form_data->count].name, key, MAX_FIELD_LENGTH - 1);
            form_data->fields[form_data->count].name[MAX_FIELD_LENGTH - 1] = '\0';
